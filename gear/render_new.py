...
def optimize(config):
    ...
    print(params['projector.active_pixels'].shape)
    #lpatterns = np.load("/scratch/wechsler/O01/patterns.npz")["patterns"][:, 380:680, :].flatten()
    lpatterns = np.load("O01_black_hollow2/patterns.npz")["patterns"].flatten()
    print(lpatterns.shape)
    params['projector.active_data'] = lpatterns
    params.update()
    vol_final = mi.render(scene, params, spp=10, integrator=integrator_final, sensor=final_sensor)
